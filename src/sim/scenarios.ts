...
export interface SimStep {
  ...
  topic: string;
  payload?: unknown;
  /** Component publishing this step — used as `user_properties.clientId`
   * would be on a live broker so simulated flows mirror real source routing. */
  publisher?: string;
}
...
const simpleRequestResponse: Scenario = {
  ...
  steps: [
    ...
    t(400, `${NS}/a2a/v1/agent/request/OrchestratorAgent`,
      { id: "task-100", params: userMsg("What's the weather in Tokyo right now?") },
      "webui-1"),
    t(150, `${NS}/a2a/v1/gateway/status/webui-1/task-100`,
      { params: { status: "working", ...agentMsg("Routing to WeatherAgent…") } },
      "OrchestratorAgent"),
    t(200, `${NS}/a2a/v1/agent/request/WeatherAgent`,
      { id: "sub-100a", params: userMsg("Current weather for Tokyo, Japan in metric units.") },
      "OrchestratorAgent"),
    t(150, `${NS}/a2a/v1/agent/status/OrchestratorAgent/sub-100a`,
      { params: { status: "working", ...agentMsg("Fetching from weather API…") } },
      "WeatherAgent"),
    t(400, `${NS}/a2a/v1/agent/response/OrchestratorAgent/sub-100a`,
      { result: { status: "completed", ...agentMsg("Tokyo: 22°C, partly cloudy, light breeze from the east.") } },
      "WeatherAgent"),
    t(150, `${NS}/a2a/v1/gateway/response/webui-1/task-100`,
      { result: { status: "completed", ...agentMsg("It's 22°C and partly cloudy in Tokyo right now.") } },
      "OrchestratorAgent"),
  ],
};
...
const multiAgentDelegation: Scenario = {
  ...
  steps: [
    ...
    t(300, `${NS}/a2a/v1/agent/request/OrchestratorAgent`,
      { id: "task-200", params: userMsg("Summarize last quarter's top-selling products by region.") },
      "slack-1"),
    t(120, `${NS}/a2a/v1/gateway/status/slack-1/task-200`,
      { params: { status: "working", ...agentMsg("Planning: need SQL pull, then summarize.") } },
      "OrchestratorAgent"),
    // Orchestrator delegates to the sub-orchestrator…
    t(200, `${NS}/a2a/v1/agent/request/SAOrchestratorAgent`,
      { id: "sub-200a", params: userMsg("Get top products per region for Q4, then summarize for an exec audience.") },
      "OrchestratorAgent"),
    t(150, `${NS}/a2a/v1/agent/status/OrchestratorAgent/sub-200a`,
      { params: { status: "working", ...agentMsg("Splitting work across SqlAgent and SummaryAgent.") } },
      "SAOrchestratorAgent"),
    // …which in turn fans out to leaf agents.
    t(250, `${NS}/a2a/v1/agent/request/SqlAgent`,
      { id: "sub-200b", params: userMsg("SELECT region, product, SUM(revenue) FROM sales WHERE quarter='Q4' GROUP BY region, product ORDER BY 3 DESC LIMIT 5 per region.") },
      "SAOrchestratorAgent"),
    t(120, `${NS}/a2a/v1/agent/status/SAOrchestratorAgent/sub-200b`,
      { params: { status: "working", ...agentMsg("Query running…") } },
      "SqlAgent"),
    t(250, `${NS}/a2a/v1/agent/request/SummaryAgent`,
      { id: "sub-200c", params: userMsg("Condense the SQL result into 3 bullet points for a board memo.") },
      "SAOrchestratorAgent"),
    t(120, `${NS}/a2a/v1/agent/status/SAOrchestratorAgent/sub-200c`,
      { params: { status: "working", ...agentMsg("Waiting for SQL results, then summarizing.") } },
      "SummaryAgent"),
    t(400, `${NS}/a2a/v1/agent/response/SAOrchestratorAgent/sub-200b`,
      { result: { status: "completed", ...agentMsg("Top sellers: EMEA — Solar kits; APAC — Smart sensors; AMER — Battery packs. Rows: 15.") } },
      "SqlAgent"),
    t(150, `${NS}/a2a/v1/agent/response/SAOrchestratorAgent/sub-200c`,
      { result: { status: "completed", ...agentMsg("• EMEA leans renewables\n• APAC drives IoT\n• AMER hardware-heavy") } },
      "SummaryAgent"),
    t(150, `${NS}/a2a/v1/agent/response/OrchestratorAgent/sub-200a`,
      { result: { status: "completed", ...agentMsg("Compiled regional Q4 summary ready for the gateway.") } },
      "SAOrchestratorAgent"),
    t(180, `${NS}/a2a/v1/gateway/response/slack-1/task-200`,
      { result: { status: "completed", ...agentMsg("Q4 highlights — EMEA: renewables, APAC: IoT sensors, AMER: hardware.") } },
      "OrchestratorAgent"),
  ],
};
