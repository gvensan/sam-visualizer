...
interface CanvasProps {
  ...
  theme: Theme;
  showLabels: boolean;
  /** Window after which an agent without a fresh AgentCard heartbeat is
   * considered stale and dimmed on the canvas. */
  agentTtlMs: number;
  /** Visualization pace multiplier. >1 plays animations faster, <1 slower.
   * Applied to particle durations so the user can match the display speed
   * to either a real-time broker burst or a careful demo walkthrough. */
  speed: number;
  /** When true, incoming events are recorded into history (via the bus)
   * but skipped by the canvas — no particle animation, no broker pulse.
 ...
 */
function resolveNode(name: string | undefined, nodes: PositionedNode[]): PositionedNode | undefined {
  if (!name || GENERIC_PUBLISHERS.has(name)) return undefined;
 ...
 * been doing the work.
 */
function publisherFromTask(state: MeshState, taskId?: string): string | undefined {
...
function planFromEvent(event: A2AEvent, nodes: PositionedNode[], state: MeshState): ParticlePlan | null {
  ...
  switch (event.kind) {
    ...
    case "delegation-response": {
      ...
      if (!delegator || !source || source.id === delegator.id) return null;
...
export function Canvas({ bus, animations, renderMode, theme, showLabels, agentTtlMs, speed, paused, onSelectNode, selectedId, spotlightTask }: CanvasProps) {
  ...
  const stale = useMemo(
    () => new Set(staleAgents(bus.getState(), Date.now(), agentTtlMs)),
    [version, bus, agentTtlMs],
  );
  const nodes = useMemo(
  ...
  const catchupActiveRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const LAYERS = ["zones", "ambient", "ripples", "particles", "nodes", "labels"] as const;
  ...
  useEffect(() => {
    ...
    const maybeExitCatchup = () => {
      if (!catchupActiveRef.current) return;
    ...
    const startCatchup = (fromSeq: number) => {
      const history = bus.getHistory();
      // Find the first index whose seq is greater than fromSeq.
      let i = 0;
      while (i < history.length && (history[i].seq ?? -1) <= fromSeq) i++;
      if (i >= history.length) return;
      catchupActiveRef.current = true;
      for (; i < history.length; i++) dispatch(history[i]);
    };
    ...
    function runParticle(
      ...
      return new Promise<void>((resolve) => {
        ...
        const layerAll = viewport.select<SVGGElement>("g.particles");
        const labelLayer = viewport.select<SVGGElement>("g.labels");
        ...
        let label: d3.Selection<SVGTextElement, unknown, null, undefined> | null = null;
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      const layerAll = d3
        .select(svgRef.current)
        .select<SVGGElement>("g.viewport")
        .select<SVGGElement>("g.ambient");
      if (layerAll.empty()) return;
