...
export function resetPaletteCursor() {
    cursor = 0;
}
/**
 ...
 */
export function inferParentTaskId(state, targetAgentName, now, maxAgeMs = 30_000) {
    let best = null;
    for (const agent of state.agents.values()) {
        ...
        if (!t || t.endTime !== null)
            continue;
        if (now - t.startTime > maxAgeMs)
            continue;
        if (!best || t.startTime > best.startTime)
            best = t;
    }
    return best?.id ?? null;
}
 ...
 */
export function shortTaskId(taskId, maxLen = 8) {
