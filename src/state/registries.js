...
const isOrchestratorName = (name) => name.toLowerCase().includes("orchestrator");
function upsertGateway(state, id, ts) {
...
function upsertAgent(state, name, ts) {
...
function upsertTask(state, id, ts) {
...
function pushAgentTask(agent, taskId) {
    const list = agent.currentTaskIds ?? [];
    if (!list.includes(taskId))
        list.push(taskId);
    agent.currentTaskIds = list;
}
function popAgentTask(agent, taskId) {
    if (!agent.currentTaskIds)
        return;
 ...
 */
function colorize(state, task, targetAgentName, ts) {
    if (task.color)
        return;
...
export function applyEvent(state, event) {
    switch (event.kind) {
        ...
        case "status": {
            if (!event.gatewayId || !event.taskId)
                return state;
        ...
        case "response": {
            ...
            if (gw && gw.activeTasks > 0)
                gw.activeTasks--;
            if (task.targetAgent) {
                const a = state.agents.get(task.targetAgent);
                if (a)
                    popAgentTask(a, task.id);
            }
            return state;
        }
        case "delegation-status":
        case "delegation-response": {
            if (!event.delegatingAgent || !event.subTaskId)
                return state;
            ...
            if (!sub.parentTask) {
                ...
                if (parentId && parentId !== sub.id) {
                    ...
                    if (parent) {
                        ...
                        if (parent.color && sub.color !== parent.color)
                            sub.color = parent.color;
                        if (!parent.subTasks.includes(sub.id))
