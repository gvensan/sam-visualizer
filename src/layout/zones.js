...
function nodeWidthForCols(cols, full) {
    if (cols <= 1)
        return full;
    if (cols === 2)
        return Math.round(full * 0.68);
    if (cols === 3)
        return Math.round(full * 0.54);
    return Math.max(60, Math.round(full * 0.92 / cols));
}
/** How many rows fit between the lane label and the canvas floor. */
function maxRowsForBand(cfg) {
 ...
 */
function distributeGrid(count, cfg, anchor, centerX) {
    if (count === 0)
        return [];
    ...
    if (startY < usableTop)
        startY = usableTop;
    if (startY + totalH > usableBottom)
        startY = usableBottom - totalH;
    const out = [];
...
export function inferExternalLabel(gatewayId) {
    for (const h of EXTERNAL_HEURISTICS) {
        if (h.pattern.test(gatewayId))
            return h.label;
    }
    return "Client";
}
...
export function buildLayout({ state, stale, cfg = DEFAULT_LAYOUT }) {
    const out = [];
    const zoneCenterX = (z) => {
        const ratio = ZONES.find((zz) => zz.id === z)?.xRatio ?? 0.5;
        return ratio * cfg.width;
    };
    const gateways = [...state.gateways.values()].sort((a, b) => a.id.localeCompare(b.id));
    ...
    const brokerCenterY = cfg.topMargin + (cfg.height - 40 - cfg.topMargin) * ANCHOR_RATIO.middle;
    out.push({
        id: BROKER_NODE_ID,
        kind: "broker",
        label: "Event Broker",
