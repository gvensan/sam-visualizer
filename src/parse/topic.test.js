...
describe("parseTopic", () => {
    ...
    it("parses gateway status with gateway id and task id", () => {
        const e = parseTopic(`${NS}/a2a/v1/gateway/status/webui-7/task-abc123`, NS, T);
    ...
    it("parses gateway response with gateway id and task id", () => {
        const e = parseTopic(`${NS}/a2a/v1/gateway/response/slack-1/task-xyz`, NS, T);
        expect(e).toMatchObject({
            kind: "response",
            gatewayId: "slack-1",
            taskId: "task-xyz",
    ...
    it("parses peer delegation status with delegating agent and sub task", () => {
        const e = parseTopic(`${NS}/a2a/v1/agent/status/OrchestratorAgent/sub-1`, NS, T);
        expect(e).toMatchObject({
    ...
    it("parses peer delegation response", () => {
        const e = parseTopic(`${NS}/a2a/v1/agent/response/OrchestratorAgent/sub-2`, NS, T);
        expect(e).toMatchObject({
    ...
    it("threads the publisher (clientId) through to the parsed event", () => {
        const e = parseTopic(`${NS}/a2a/v1/agent/request/LeafAgent`, NS, T, undefined, "SAOrchestratorAgent");
        expect(e).toMatchObject({
            kind: "request",
            agentName: "LeafAgent",
